...
const uncapitalize = <T extends string>(s: T) =>
	(s[0].toLowerCase() + s.slice(1)) as Uncapitalize<T>;

export const toQuery = <T extends Record<string, any>, E extends keyof T = never>(
	d: T,
	excludes: E[] = []
) =>
...
export const toWhere = <T extends Record<string, any>, E extends keyof T = never>(
	query: T,
	excludes: E[] = []
) =>
...
export const toPrisma = <T extends Record<string, any>, E extends keyof T = never>(
	...
	({
		where: Object.keys(query).length
			? toWhere(query as Record<string, any>, excludes as string[])
			: undefined,
		orderBy: orderBy
			? {
					[orderBy]: order,
